// Vendored from mortspace/Deskfolio (MIT) — https://github.com/mortspace/Deskfolio
// Content (name, bio, skills, journal, project blurbs, GitHub link, avatar) personalized
// for Đức Thành; layout/physics/interaction logic left as upstream wrote it.
/* eslint-disable react-hooks/set-state-in-effect -- vendored third-party interaction logic, left as upstream wrote it */
import { createContext, useCallback, useContext, useEffect, useId, useMemo, useRef, useState } from 'react'
import { AnimatePresence, motion, useReducedMotion, type Variants } from 'motion/react'
import { DeskFolio } from './lib/DeskFolio'
import { DialSlider } from './lib/DialSlider'
import { haptic } from './lib/haptics'
import { DF_SCRIBBLES } from './lib/dfScribble'
import './lib/deskfolio.css'
import { identity, objective, skillGroups, experience as cvExperience, projects as cvProjects } from '@/content/cv-data'
...
function EditableNameTag({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    const el = ref.current
    if (el && !el.textContent) el.textContent = 'Đức Thành'
  }, [])
...
const DEFAULT_DEV_ACTIVITY_LINK = links.github !== '#' ? links.github : 'https://github.com'
        ...
        <span className="df-ipad-screen">
          ...
          <svg className="df-ipad-ui" viewBox="0 0 264 201" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
            ...
            <g className="df-ui-tags">
              ...
              <rect x="61" y="182" width="43" height="15" rx="7.5" />
              <text x="82.5" y="192.6">Next.js</text>
              <rect x="109" y="182" width="33" height="15" rx="7.5" />
              <text x="125.5" y="192.6">TS</text>
            </g>
          </svg>
        </span>
      </span>
  ...
  <div className="df-cover">
    ...
    <div className="df-cover-label">
      <span className="df-cover-mark" aria-hidden="true">
        <span className="df-cover-mono">ĐT</span>
...
const DESK_PROJECT_IDS = ['aeon', 'chatbot', 'escrow', 'cryptos'] as const
const deskProjects = DESK_PROJECT_IDS.map((id) => cvProjects.find((p) => p.id === id)!)
const [aeonRole, escrowRole, sppRole] = [cvExperience[0], cvExperience[2], cvExperience[3]]

function getPortfolioPages(locale: Locale): React.ReactNode[] {
  const vi = locale === 'vi'
  return [
  // spread 1 left: about
  <div className="df-page" key="p0">
    ...
    <h3 className="df-hello">{vi ? 'giới thiệu' : 'about me'} <span className="df-heart" aria-hidden="true">♡</span></h3>
    <div className="df-about-head">
      <span className="df-avatar"><img src="/avatar.svg" alt={identity.name} loading="lazy" decoding="async" /></span>
      <div>
        <Editable className="df-name" placeholder={vi ? 'tên của bạn' : 'your name'} initial={identity.name} />
        <span className="df-role"><LbSparkle /> {vi ? 'frontend developer' : 'front-end developer'}</span>
      </div>
    </div>
    <div className="df-card">
      <Editable
        className="df-edit--body"
        placeholder={vi ? 'đôi dòng về bạn…' : 'a little about you…'}
        initial={objective[locale]}
      />
    </div>
    <p className="df-label df-label--push">{vi ? 'hiện tại…' : 'currently…'}</p>
    <span className="df-date"><LbHeart /> {vi ? 'đang mở cho cơ hội mới' : 'open to new opportunities'}</span>
  </div>,
  ...
  <div className="df-page" key="p1">
    ...
    <h3 className="df-hello df-hello--lilac">{vi ? 'kỹ năng' : 'skills'}</h3>
    <div className="df-card df-card--lilac">
      <p className="df-card-title"><LbSparkle /> {vi ? 'công cụ mình dùng' : 'i build with'}</p>
      <ul className="df-tags">
        {[...skillGroups[0].items, ...skillGroups[1].items.slice(1), skillGroups[3].items[0]].map((tag) => (
          <li className="df-tag" key={tag}>{tag}</li>
        ))}
      </ul>
    </div>
    <div className="df-card">
      <p className="df-card-title"><LbHeart /> {vi ? 'mình chú trọng' : 'i care about'}</p>
      <ul className="df-tags df-tags--blush">
        <li className="df-tag">{vi ? 'hiệu năng' : 'performance'}</li>
        <li className="df-tag">{vi ? 'kiến trúc mở rộng' : 'scalable architecture'}</li>
        <li className="df-tag">{vi ? 'clean code' : 'clean code'}</li>
        <li className="df-tag">{vi ? 'mentoring' : 'mentoring'}</li>
      </ul>
    </div>
  </div>,
  ...
  <div className="df-page" key="p4">
    <h3 className="df-hello">{vi ? 'kinh nghiệm' : 'experience'}</h3>
    <ul className="df-xp">
      <li className="df-xp-row">
        <span className="df-xp-dot" aria-hidden="true" />
        <div>
          <p className="df-xp-role">Frontend Developer</p>
          <p className="df-xp-meta">Peasoft Việt Nam · {aeonRole.period[locale]}</p>
        </div>
      </li>
      <li className="df-xp-row">
        <span className="df-xp-dot" aria-hidden="true" />
        <div>
          <p className="df-xp-role">Frontend Developer</p>
          <p className="df-xp-meta">Joom Solutions · {escrowRole.period[locale]}</p>
        </div>
      </li>
      <li className="df-xp-row">
        <span className="df-xp-dot" aria-hidden="true" />
        <div>
          <p className="df-xp-role">Frontend Developer</p>
          <p className="df-xp-meta">Agiletech · {sppRole.period[locale]}</p>
        </div>
      </li>
    </ul>
  </div>,
  ...
  <div className="df-page" key="p5">
    ...
    <h3 className="df-hello df-hello--lilac">{vi ? 'liên hệ' : 'say hi'} <span className="df-heart" aria-hidden="true">♡</span></h3>
    <Editable
      className="df-edit--body"
      placeholder={vi ? 'một lời chào…' : 'a friendly note…'}
      initial={vi ? 'Có dự án, câu hỏi, hay chỉ muốn trò chuyện về nghề? Mình rất sẵn lòng.' : "Got a project, a question, or just want to talk shop? I'd love to hear from you."}
    />
    <ul className="df-links">
      <li>
        <a className="df-link" href={links.github} target="_blank" rel="noreferrer"><IconGitHub /> GitHub</a>
      </li>
      <li>
        <a className="df-link" href={links.linkedin} target="_blank" rel="noreferrer"><IconLinkedIn /> LinkedIn</a>
      </li>
      <li>
        <a className="df-link" href={`mailto:${identity.email}`}><IconMail /> Email</a>
      </li>
    </ul>
    <p className="df-sig">{vi ? 'chào mình bất cứ lúc nào' : 'say hi anytime'} <span className="df-heart" aria-hidden="true">♡</span></p>
  </div>,
  ...
  <div className="df-page" key="p6">
    ...
    <h3 className="df-hello">{vi ? 'về cuốn sổ này' : 'made by hand'}</h3>
    <div className="df-card">
      <Editable
        className="df-edit--body"
    ...
    <p className="df-sig">{vi ? 'cảm ơn bạn đã ghé qua' : 'thanks for the curiosity'} <span className="df-heart" aria-hidden="true">♡</span></p>
  </div>,
  // spread 4 right: closing finale
  <div className="df-page df-thanks" key="p7">
    <LbSparkle className="df-thanks-spark" />
    <p className="df-thanks-title">{vi ? 'cảm ơn đã đọc' : 'thanks for reading'} <span className="df-heart" aria-hidden="true">♡</span></p>
    <span className="df-thanks-sub">{vi ? 'cùng làm điều gì đó nhé' : "let's make something"}</span>
  </div>,
  ]
}
...
export function DeskFolioPage() {
  ...
  const portfolioPages = useMemo(() => getPortfolioPages(locale), [locale])
  ...
  const cover = PORTFOLIO_COVER
  // portfolio has no back cover
  const backCover = undefined
  ...
  return (
    <>
      ...
      {compact ? (
        <p className="tagline">
          {locale === 'vi'
            ? 'Trải nghiệm này hợp nhất trên desktop. Bản desktop mở khoá đầy đủ tương tác, tuỳ biến và hiệu ứng.'
